...
import org.stampar.lib.reality.PhysicalPlace;
...
//	private MixVector oMarker = new MixVector();
	...
	public LocalMarker(String id, String title, double latitude, double longitude, double altitude, String link, int type, int colour) {
		...
		this.dongs_ID = id;
		//this.ID = id;
		// 여기서 생성되는 URL은 마커가 찍혔을 때 이동하는 넘 입니당..
		//Log.v("URL_CHECK", URL);
	}
...
//	}
	...
	public void draw(PaintScreen dw, Bitmap[] bitArray, Movie[] mv, SharedPreferences pref) {
		drawCircle(dw, bitArray, mv, pref);
		drawTextBlock(dw);
	}
