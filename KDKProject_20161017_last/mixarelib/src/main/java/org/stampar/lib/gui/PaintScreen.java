...
public class PaintScreen implements Parcelable{
	...
	// 피카츄를 띄우기 위해 만든 함수임.
	public void paintImg(Bitmap target,
			float x, float y, float scale)
	{
		//canvas.save();
		//canvas.rotate(scale, x,y);
		canvas.drawBitmap(target, x,y, null);
		//target.draw(canvas);
		//canvas.restore();
	}
	// 이 함수는 프레임을 씌우기 위한 것임.
	public void paintImg(Drawable target)
	{
		target.draw(canvas);
	}

	public void paintGif(Movie target, float x, float y, float scale)
	{
		...
		int relTime = (int)((now - mvStart) % target.duration());
		target.setTime(relTime);
